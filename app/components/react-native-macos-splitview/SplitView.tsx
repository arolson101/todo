import React, { Children, FunctionComponent, ReactElement, useMemo, useRef } from 'react'
import { Animated, PanResponder, StyleSheet, View, ViewProps, ViewStyle } from 'react-native'
...
const SplitView: FunctionComponent<Props> = ({
  ...
  const clampedValue = useRef(new AnimatedClamp(vertical ? pan.x : pan.y, min, max)).current
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: () => {
        const axisValue = (vertical ? pan.x : pan.y) as unknown as RawAnimatedValue
        axisValue._offset = clamp(axisValue._offset, min - axisValue._value, max - axisValue._value)
  ...
  const verticalStyle = useMemo<ViewStyle>(() => ({ flexDirection: vertical ? 'row' : 'column' }), [vertical])
  ...
  return (
    <View {...otherViewProps} style={[styles.splitView, verticalStyle, style]}>
      ...
      <View style={[styles.divider, dividerVerticalStyle, dividerStyle]} {...panResponder.panHandlers} />
      {panes[1]}
    </View>
  )
}
